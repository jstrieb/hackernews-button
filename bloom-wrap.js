...
async function updateBloom(force = false) {
  ...
  // incorrect
  if (info.date_generated <= window.bloom.last_generated) {
  ...
  // last_generated date to combine with
    ...
    sorted.sort((x, y) => Math.abs(x - window.bloom.last_generated) -
      Math.abs(y - window.bloom.last_generated));
...
function addBloom(bloom, url) {
  if (!bloom || !bloom.addr) {
    loadBloom();
    throw "Bloom filter is not allocated!";
    return false;
  }
